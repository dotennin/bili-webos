...
async function main() {
  ...
  ws.on('open', () => {
    ...
    setTimeout(() => {
      ws.send(JSON.stringify({ id: msgId++, method: 'Performance.getMetrics' }));
    }, 2000);
  ...
  ws.on('message', (data) => {
    ...
    if (msg.id === 100) {
      ...
      } catch { console.log('  Raw:', msg.result?.result?.value); }
    }
    if (msg.result?.metrics) {
      console.log('\n=== Performance Metrics ===');
      msg.result.metrics.forEach(m => {
        if (['JSHeapUsedSize', 'JSHeapTotalSize', 'Nodes', 'LayoutCount', 'RecalcStyleCount', 'TaskDuration'].includes(m.name)) {
          const val = m.name.includes('Heap') ? (m.value / 1048576).toFixed(1) + ' MB' : m.value;
