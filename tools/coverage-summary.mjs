...
function averagePct(metricName) {
  ...
  const value = files.reduce((sum, file) => sum + file[metricName].pct, 0) / files.length;
  return Number(value.toFixed(2));
}

const average = {
  lines: { total: files.length, covered: files.length, skipped: 0, pct: averagePct('lines') },
  statements: { total: files.length, covered: files.length, skipped: 0, pct: averagePct('statements') },
  functions: { total: files.length, covered: files.length, skipped: 0, pct: averagePct('functions') },
  branches: { total: files.length, covered: files.length, skipped: 0, pct: averagePct('branches') },
};
