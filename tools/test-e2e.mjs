...
function signWbi(params, mixinKey) {
  ...
  const q = Object.keys(p).sort().map(k =>
    k + '=' + encodeURIComponent(String(p[k]).replace(/[!'()*]/g, ''))
  ).join('&');
  const w_rid = crypto.createHash('md5').update(q + mixinKey).digest('hex');
...
async function run() {
  ...
  let mixinKey;
  try {
    const nav = await fetchJSON(`${PROXY}/proxy/api.bilibili.com/x/web-interface/nav`);
    const imgKey = nav.data?.wbi_img?.img_url?.split('/').pop().split('.')[0];
    const subKey = nav.data?.wbi_img?.sub_url?.split('/').pop().split('.')[0];
    if (imgKey && subKey) {
      mixinKey = getMixinKey(imgKey + subKey);
      ok('WBI keys', `imgKey=${imgKey.slice(0, 8)}... subKey=${subKey.slice(0, 8)}...`);
    } else {
  ...
  console.log('\n[Login]');
  try {
    const qr = await fetchJSON(`${PROXY}/proxy/passport.bilibili.com/x/passport-login/web/qrcode/generate`);
  ...
  try {
    ...
    const pop = await fetchJSON(`${PROXY}/proxy/api.bilibili.com/x/web-interface/popular?${q}`);
    if (pop.code === 0 && pop.data?.list?.length > 0) {
      ok('Popular videos', `${pop.data.list.length} videos, first: ${pop.data.list[0].title?.slice(0, 30)}...`);
    } else {
  ...
  try {
    const q = signWbi({ fresh_idx: 1, fresh_idx_1h: 1, fresh_type: 4, ps: 5 }, mixinKey);
    const rec = await fetchJSON(`${PROXY}/proxy/api.bilibili.com/x/web-interface/wbi/index/top/feed/rcmd?${q}`);
  ...
  // Test 6: Search
  try {
    const q = signWbi({ search_type: 'video', keyword: '猫', page: 1, page_size: 3, duration: 0, order: '', tids: 0 }, mixinKey);
    const search = await fetchJSON(`${PROXY}/proxy/api.bilibili.com/x/web-interface/search/type?${q}`);
  ...
  try {
    ...
    const info = await fetchJSON(`${PROXY}/proxy/api.bilibili.com/x/web-interface/view?${q}`);
    if (info.code === 0 && info.data?.cid) {
      testCid = info.data.cid;
      ok('Video info', `cid=${testCid} title="${info.data.title?.slice(0, 30)}..."`);
    } else {
  ...
  if (testCid) {
    try {
      const q = signWbi({ bvid: 'BV1GvXKBMEAb', cid: testCid, fnval: 4048, fnver: 0, fourk: 1, platform: 'pc', qn: 80 }, mixinKey);
      const play = await fetchJSON(`${PROXY}/proxy/api.bilibili.com/x/player/playurl?${q}`);
      if (play.code === 0 && play.data?.dash) {
        ...
        if (hasVideo && hasAudio && hasSeg) {
          ok('Play URL (DASH)', `${dash.video.length} video + ${dash.audio.length} audio streams, quality=${play.data.quality}`);
        } else {
          fail('Play URL (DASH)', `Missing baseUrl or SegmentBase: video=${hasVideo} audio=${hasAudio} seg=${hasSeg}`);
        }
      } else {
  ...
  if (testCid) {
    try {
      const res = await fetch(`${PROXY}/proxy/api.bilibili.com/x/v1/dm/list.so?oid=${testCid}`);
      const text = await res.text();
      const count = (text.match(/<d /g) || []).length;
      if (count > 0) {
        ok('Danmaku', `${count} items parsed from XML`);
      } else {
        fail('Danmaku', `No <d> tags found, response starts with: ${text.slice(0, 80)}`);
      }
    } catch (e) {
  ...
  try {
    const q = signWbi({ bvid: 'BV1GvXKBMEAb' }, mixinKey);
    const rel = await fetchJSON(`${PROXY}/proxy/api.bilibili.com/x/web-interface/archive/related?${q}`);
  ...
  try {
    ...
    const rank = await fetchJSON(`${PROXY}/proxy/api.bilibili.com/x/web-interface/ranking/v2?${q}`);
  ...
  console.log('\n[Video CDN]');
  try {
    const q = signWbi({ bvid: 'BV1GvXKBMEAb', cid: testCid, fnval: 4048, fnver: 0, fourk: 1, platform: 'pc', qn: 32 }, mixinKey);
    const play = await fetchJSON(`${PROXY}/proxy/api.bilibili.com/x/player/playurl?${q}`);
    const videoUrl = play.data?.dash?.video?.[play.data.dash.video.length - 1]?.baseUrl;
    if (videoUrl) {
      const u = new URL(videoUrl);
      const proxyUrl = `${PROXY}/proxy/${u.host}${u.pathname}${u.search}`;
      const segRes = await fetch(proxyUrl, { method: 'GET', headers: { Range: 'bytes=0-1023' } });
      if (segRes.status === 200 || segRes.status === 206) {
        const buf = await segRes.arrayBuffer();
        ok('Video segment proxy', `Got ${buf.byteLength} bytes, status=${segRes.status}`);
      } else {
