...
export function expectedIpkName({ appId, version }) {
  if (!appId || !version) {
    throw new Error('appId and version are required to derive the packaged IPK name');
  }
...
export function renderReleaseManifest({ template, sha256 }) {
  ...
  if (!template.ipkUrl || !template.ipkUrl.includes('{version}')) {
    throw new Error('Manifest template ipkUrl must include a {version} placeholder');
  }
  if (!sha256) {
    throw new Error('A SHA-256 digest is required to render the final manifest');
  }
...
export async function generateReleaseManifest({ templatePath, distDir, outputPath }) {
...
async function main() {
  ...
  const outputPath = args.output ?? path.join(distDir, 'com.biliwebos.app.manifest.release.json');

  const result = await generateReleaseManifest({ templatePath, distDir, outputPath });
