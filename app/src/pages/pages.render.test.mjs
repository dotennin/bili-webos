...
import { React, render, textOf, flush, interact } from '../test/reactTestUtils.mjs';
...
beforeEach(() => {
  api = {
    ...
    qrCodeGenerate: mock(async () => ({ data: { url: 'https://qr', qrcode_key: 'key-1' } })),
  ...
  mock.module('../components/VideoGrid', () => ({
    default(props) {
      ...
      return React.createElement('mock-grid', { count: props.videos?.length || 0, onSelect: props.onSelect }, props.videos?.map((v) => v.title).join('|') || 'empty');
    },
  }));
  mock.module('../components/OSKey', () => ({
    default(props) {
      oskKeys.push(props);
      return React.createElement('mock-key', { label: props.label, onPress: props.onPress }, props.label);
    },
  }));
...
describe('page rendering', () => {
  test('FavoritesPage covers logged-out, loading, and loaded states', async () => {
    const { default: FavoritesPage } = await importFresh('./FavoritesPage.jsx');

    const noUser = await render(React.createElement(FavoritesPage, { onPlayVideo() {} }));
    expect(textOf(noUser.toJSON())).toContain('请先登录');

    api.getFavFolders.mockImplementationOnce(async () => ({ data: { list: [{ id: 7 }] } }));
    api.getFavList.mockImplementationOnce(async () => ({
      data: { medias: [{ bvid: 'BV1', title: '收藏视频', cover: 'p', duration: 12, upper: { name: 'UP' }, cnt_info: { play: 99 } }] },
    }));
    const page = await render(React.createElement(FavoritesPage, { userMid: 1, onPlayVideo() {} }));
    await flush();
    expect(api.getFavFolders).toHaveBeenCalledWith(1);
    expect(api.getFavList).toHaveBeenCalledWith(7, 1, 24);
    expect(videoGridCalls.at(-1).videos[0]).toMatchObject({ bvid: 'BV1', title: '收藏视频', owner: { name: 'UP' } });
    expect(textOf(page.toJSON())).toContain('收藏视频');
  });

  test('HistoryPage handles login errors, api errors, and successful mapping', async () => {
    const { default: HistoryPage } = await importFresh('./HistoryPage.jsx');

    api.getHistory.mockImplementationOnce(async () => ({ code: -101 }));
    const needLogin = await render(React.createElement(HistoryPage, { onPlayVideo() {} }));
    await flush();
    ...
    const failed = await render(React.createElement(HistoryPage, { onPlayVideo() {} }));
    await flush();
    expect(textOf(failed.toJSON())).toContain('网络异常');

    api.getHistory.mockImplementationOnce(async () => ({
      data: { list: [{ history: { bvid: 'BV2', cid: 9 }, title: '历史视频', cover: 'c', duration: 33, progress: 10, author_name: '作者' }] },
    }));
    const ok = await render(React.createElement(HistoryPage, { onPlayVideo() {} }));
    await flush();
    expect(videoGridCalls.at(-1).videos[0]).toMatchObject({ bvid: 'BV2', cid: 9, progress: 10 });
    expect(textOf(ok.toJSON())).toContain('历史视频');
  });

  test('LoginPage renders QR, handles scanned, success, expired, and error states', async () => {
    const { default: LoginPage } = await importFresh('./LoginPage.jsx');
    const logins = [];

    api.qrCodeGenerate.mockImplementationOnce(async () => ({ data: { url: 'https://qr-1', qrcode_key: 'key-1' } }));
    const renderer = await render(React.createElement(LoginPage, { onLogin: () => logins.push('ok') }), {
      createNodeMock: (element) => (element.type === 'canvas' ? { tag: 'canvas' } : null),
    });
    await flush();
    expect(qrCalls[0][1]).toBe('https://qr-1');
    expect(textOf(renderer.toJSON())).toContain('请使用哔哩哔哩手机客户端扫描二维码');
    ...
    api.qrCodePoll.mockResolvedValueOnce({ data: { code: 0, refresh_token: 'refresh-1' } });
    await interact(() => intervals[0].fn());
    ...
    const errorRenderer = await render(React.createElement(LoginPage, { onLogin() {} }), {
      createNodeMock: (element) => (element.type === 'canvas' ? { tag: 'canvas' } : null),
    });
    await flush();
  ...
  test('SearchPage updates keyword, runs search, and renders empty/results states', async () => {
    const { default: SearchPage } = await importFresh('./SearchPage.jsx');
    api.searchVideo.mockImplementationOnce(async () => ({ data: { result: [] } }));
    const keyByLabel = (label) => oskKeys.filter((item) => item.label === label).at(-1);

    const renderer = await render(React.createElement(SearchPage, { onPlayVideo() {} }));
    ...
    api.searchVideo.mockImplementationOnce(async () => ({
      data: { result: [{ title: '<em>结果</em>', pic: 'p', bvid: 'BV3', author: '作者', play: 12, duration: '01:20' }] },
    }));
    await interact(() => keyByLabel('搜索').onPress());
    expect(videoGridCalls.at(-1).videos[0]).toMatchObject({ title: '结果', owner: { name: '作者' }, stat: { view: 12 } });
    expect(textOf(renderer.toJSON())).toContain('结果');
  });

  test('SettingsPage toggles danmaku, logs out, and renders watch history', async () => {
    const { default: SettingsPage } = await importFresh('./SettingsPage.jsx');
    api.getHistory.mockImplementationOnce(async () => ({
      data: { list: [{ history: { bvid: 'BV4', cid: 4 }, title: '最近观看', cover: 'x', duration: 90, progress: 20, author_name: 'UP' }] },
    }));
    const logs = [];
