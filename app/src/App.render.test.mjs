...
import { React, render, flush, textOf, createEventTarget, update, interact } from './test/reactTestUtils.mjs';
...
beforeEach(() => {
  ...
  navResponse = { data: { isLogin: true, mid: 1, uname: '已登录用户', face: 'avatar.png' } };
  storageState = {
    auth: { SESSDATA: 'sess' },
  };
  sidebarItems = [];
  ...
  globalThis.localStorage = {
    getItem(key) {
      if (key === 'bili_auth') return storageState.auth ? JSON.stringify(storageState.auth) : null;
      return null;
    },
    setItem(key, value) {
  ...
  mock.module('./components/SidebarItem', () => ({
    default(props) {
      ...
      return React.createElement('sidebar-item', { onClick: props.onSelect, active: props.active }, props.label);
    },
  }));
  for (const page of ['LoginPage', 'HomePage', 'SearchPage', 'SettingsPage']) {
    mock.module(`./pages/${page}.jsx`, () => ({
      default(props) {
...
test('App loads user info, routes pages, handles cast commands, login, logout, and back behavior', async () => {
  const { default: App } = await importFresh('./App.jsx');
  const renderer = await render(React.createElement(App));
  await flush();

  expect(textOf(renderer.toJSON())).toContain('已登录用户');
  expect(pageProps.some((entry) => entry.page === 'HomePage' && entry.props.mode === 'recommend')).toBe(true);
  timers[0].fn();

  await interact(() => sidebarItems.find((item) => item.label === '热门').onSelect());
  expect(pageProps.some((entry) => entry.page === 'HomePage' && entry.props.mode === 'hot')).toBe(true);

  await interact(() => sidebarItems.find((item) => item.label === '搜索').onSelect());
  expect(pageProps.some((entry) => entry.page === 'SearchPage')).toBe(true);

  const recommendEntry = pageProps.find((entry) => entry.page === 'HomePage' && entry.props.mode === 'recommend');
  await interact(() => recommendEntry.props.onPlayVideo({ roomid: 77, isLive: true, title: '直接直播' }));
  ...
  await interact(() => castSubscription({
    kind: 'command',
    command: { type: 'play', contentType: 'video', bvid: 'BV9', cid: 2, title: '投屏视频', seekTs: 15 },
  }));
  expect(playerProps.video.video).toMatchObject({ bvid: 'BV9', title: '投屏视频', progress: 15, fromCast: true });
  ...
  await flush();
  await interact(() => sidebarItems.find((item) => item.label === '关注').onSelect());
  expect(playerProps.login).not.toBeNull();

  await interact(() => playerProps.login.onLogin());
  expect(pageProps.some((entry) => entry.page === 'HomePage' && entry.props.mode === 'recommend')).toBe(true);

  await interact(() => sidebarItems.find((item) => item.label === '我的').onSelect());
  const settingsEntry = pageProps.filter((entry) => entry.page === 'SettingsPage').at(-1);
  await interact(() => settingsEntry.props.onLogout());
  expect(storageState.auth).toBeNull();

  await interact(() => castSubscription({
    kind: 'command',
    command: { type: 'play', contentType: 'live', roomId: 100, title: '直播间' },
  }));
