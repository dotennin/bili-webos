  ...
  assert.match(
    ...
    new RegExp(
      buildProxyUrl(
        proxyBase,
        'd1--ov-gotcha207.bilivideo.com',
        '/live-bvc/950683/live_12939237_4978623_2500/1778568917.m4s',
      ),
    ),
  );
});
...
test('rewrites absolute hls urls but leaves comments untouched', () => {
  ...
  assert.ok(
    rewritten.includes(
      buildProxyUrl(proxyBase, 'cdn.bilivideo.com', '/live/alt.m3u8?bar=2'),
    ),
  );
  assert.ok(
    rewritten.includes(
      buildProxyUrl(proxyBase, 'example.bilivideo.com', '/live/seg-1.ts'),
    ),
  );
});
