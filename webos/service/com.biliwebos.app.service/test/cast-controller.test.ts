...
test('unsupported playurl payload is ignored without clobbering active session', () => {
  ...
  controller.handleCommand(
    'session-1',
    'Play',
    JSON.stringify({ aid: 1, cid: 2, bvid: 'BV1' }),
  );
...
test('handles pause/resume/stop/switchDanmaku actions and detach', () => {
  ...
  assert.deepEqual(controller.handleCommand('s1', 'Pause', '{}'), {
    type: 'pause',
  });
  assert.deepEqual(controller.handleCommand('s1', 'Resume', '{}'), {
    type: 'resume',
  });
  assert.deepEqual(
    controller.handleCommand(
    ...
    { type: 'switchDanmaku', open: true },
  );
  assert.deepEqual(controller.handleCommand('s1', 'Stop', '{}'), {
    type: 'stop',
  });
...
test('gracefully handles invalid json and malformed playurl', () => {
  ...
  const malformed = controller.handleCommand(
    's1',
    'PlayUrl',
    JSON.stringify({
      url: 'not-a-valid-url',
