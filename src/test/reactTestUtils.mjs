...
function installCreateNodeMock(options) {
  ...
  const originalCreateElement = renderDocument.createElement.bind(renderDocument);
  const originalCreateElementNS = renderDocument.createElementNS?.bind(renderDocument);
  ...
  renderDocument.createElement = function patchedCreateElement(tagName, elementOptions) {
    const element = originalCreateElement(tagName, elementOptions);
    return applyIfNeeded(element, tagName);
  };
  if (originalCreateElementNS) {
    renderDocument.createElementNS = function patchedCreateElementNS(namespaceURI, qualifiedName, optionsArg) {
      const element = originalCreateElementNS(namespaceURI, qualifiedName, optionsArg);
...
function createRenderer(container, root, element, options) {
  return {
    ...
    toJSON() {
      ...
      return Array.from(container.childNodes).map(serializeNode).filter(Boolean);
    },
    unmount() {
      act(() => {
        root.unmount();
...
export function textOf(node) {
  ...
  if (renderWindow?.Node && node instanceof renderWindow.Node) return node.textContent || '';
...
export function createVideoMock() {
  ...
  const mockVideo = {
    ...
    __attachElement(element) {
      ...
      for (const name of ['addEventListener', 'removeEventListener', 'play', 'pause', 'scrollIntoView']) {
        bindMethod(name);
      }
      for (const key of ['currentTime', 'duration', 'paused', 'readyState', 'ended', 'playCalls', 'pauseCalls']) {
