...
// Simple MD5 implementation (no external deps needed)
function md5(string) {
...
const WBI_KEYS_TTL = 600000; // 10 min

export async function getWbiKeys(apiFetch) {
  const now = Date.now();
  if (
    cachedWbiKeys &&
    now >= wbiKeysFetchedAt &&
    now - wbiKeysFetchedAt < WBI_KEYS_TTL
  ) {
    return cachedWbiKeys;
  }

  const data = await apiFetch('/x/web-interface/nav');
...
export function signWbi(params, imgKey, subKey) {
  ...
  // Sort by key
  const sorted = Object.keys(newParams).sort();
  const query = sorted
    .map((k) => {
      let v = String(newParams[k]);
      // Filter characters
      v = v.replace(/[!'()*]/g, '');
