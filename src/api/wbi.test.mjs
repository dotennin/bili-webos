...
import { getWbiKeys, signWbi, md5 } from './wbi.js';
...
describe('wbi helpers', () => {
  ...
  it('getWbiKeys refetches after ttl and handles missing urls', async () => {
    ...
    const apiFetch = async () => {
      count += 1;
      if (count === 1) {
        return { data: { wbi_img: { img_url: 'https://x/one.png', sub_url: 'https://x/two.png' } } };
      }
      return { data: {} };
    };
  ...
  it('signWbi sorts params, strips special chars and appends w_rid', () => {
    ...
    const result = signWbi({ z: "x!'()*", a: '1 2' }, 'abcdefghijklmnopqrstuvwxyz012345', '6789abcdefghijklmnopqrstuvwxyzAB');
